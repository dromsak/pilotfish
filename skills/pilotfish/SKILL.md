...
| Role | Delegate when |
|---|---|
| `pilotfish:scout` | Any search, lookup, or "where/how is X" reconnaissance. Never the built-in `Explore` — it inherits your main-session model, so every search it runs bills at frontier rates. Nothing blocks it; you must not use it. `pilotfish:scout` is the same role pinned to Sonnet at low effort. |
...
- **These rules judge one decision at a time; recurrence is their blind spot.** Small same-shape tasks that arrive one at a time — review findings with a known remedy, templated writing (PR bodies, release accounting, plan documents), single-file follow-up fixes — each pass the "faster to just do it" test individually, and a session can spend hours executing them serially at frontier rates without any single decision being wrong. After doing the same shape of small task directly about three times, if more instances are still arriving, stop treating each as a one-off: batch the remainder into one `pilotfish:executor` or `pilotfish:mech-executor` brief and keep only per-item triage. An already-diagnosed review finding is batchable execution, not a fresh unknown bug — "the context is already hot" justifies the first fix, not the twentieth.
- Non-trivial changes get a fresh-context `pilotfish:verifier` pass before you report them done; prefer that over self-review. Scope that pass to feature or PR closure: inside a fix loop, re-running the project's test gate yourself is evidence enough to continue, and a verifier pass per micro-fix multiplies frontier cost without adding independent information.
